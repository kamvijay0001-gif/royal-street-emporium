...
function AboutPage() {
  ...
  return (
    <StoreLayout>
      <PageHeader title="About Us" subtitle={String(store?.["tagline"] ?? "Premium Street Fashion")} />
      <div className="mx-auto max-w-3xl space-y-8 px-4 py-14">
        ...
        <Reveal>
          <p className="text-sm leading-relaxed text-muted-foreground">
            ...
            days of delivery. If you're nearby, come and try things on — we're open {String(store?.["hours"] ?? "seven days a week")}.
          </p>
        </Reveal>
        <Reveal>
          <div className="border border-border p-6">
            <p className="eyebrow text-muted-foreground">Visit the store</p>
            <p className="mt-2 text-sm">{String(store?.["address"] ?? "")}</p>
            <p className="mt-1 text-sm text-muted-foreground">{String(store?.["phone"] ?? "")}</p>
